...
class vision_1:
    ...
    def determinejointangles(self):
        ...
        self.joint3.data = -(joint3Ang - np.sign(joint3Ang) * 0.35 * abs(np.sin(
            self.joint2.data)))  # self.angleBetweenVectors(zUnitVector, vecjoint3)*np.cos(self.angleBetweenVectors(zUnitVector, vecjoint3))
        # ybr = self.vectorYB - self.vectorBR
        # self.joint4.data = -self.angleBetweenVectors2(self.vectorYB, self.vectorBR) * np.sign(ybr[0]) * np.sign(ybr[2])
