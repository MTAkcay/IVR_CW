...
class vision_2:
    ...
    def determinejointangles(self):
        ...
        unrotatedVectorBR = self.reverseJoint3(self.reverseJoint1(self.vectorBR))[0:3:2]
        horizontalYBDist = np.linalg.norm(self.vectorYB[:2])

        self.joint1.data = np.arctan2(self.vectorYB[0], -self.vectorYB[1])
        self.joint3.data = np.arctan2(horizontalYBDist, self.vectorYB[2])  # * self.detectSwitch()
        self.joint4.data = self.angleBetweenVectors2(self.vectorYB, self.vectorBR) * np.sign(unrotatedVectorBR[0])
    ...
    def detectSwitch(self):
        if (abs(self.joint1.data - self.lastJoint1) > 1) and (self.iterationNumber > 20):
            print(self.joint1.data, self.lastJoint1, -(np.pi - self.joint1.data) % (2 * np.pi))
            if self.joint1.data > 0:
                self.joint1.data -= np.pi
            else:
                self.joint1.data += np.pi
            return -1
        return 1
